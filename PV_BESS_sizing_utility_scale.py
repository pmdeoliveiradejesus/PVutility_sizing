...
@authors: Paulo De Oliveira (PDEOLIV@GMAIL.COM) y Alejandro Salas Durá
Optimal Sizing + BESS + PV  ─ Case 8760h - Utility Scale (Monteria)
"""
import numpy as np
...
#    Colombian Electricity Market      
     ...
     'Ppvu'    : 'PpvuCordobaERAS20052023_localtime.inc',  # Perfil Solar Las Vegas ERAS 2005-2023
#     'Ppvu'    : 'PpvuMonteriaTMY.inc',  # Perfil Solar Las Vegas ERAS 2005-2023     
    "periodo" : "periodo.inc",  # calendar to assingn type of contracted power 1..6 per each hour
# ----------------------
}
...
er = 1 # (exchange rate 1 Eur = 1.1 USD, average 2024)
# ── Capacity charges in Spain (USD/kW-year)
#kappa = [0, 28.79187*er, 15.07764*er, 6.55917*er, 5.17209*er, 1.93281*er, 0.91609*er] 
# ── Capacity charges in Las Vegas (USD/kW-year)
kappa = [0, 0, 0, 0, 0, 0, 0] 

Plinst  = 0                         # kW peak/installed Load DataCenter
Rmax   = 1000                       # W/m2
Area   = 2.4                        # Area module 500W
eta    = 0.2094                      # PV module efficiency
eff_c  = 0.9381                      # BESS charge inverter efficiency
eff_d  = 0.9381                      # BESS discharge inverter efficiency
DoD    = 0.9                       # Depth of Discharge
PmaxF   = 100000                    # límit frontier export/import
...
# CAPEX_BESS = 0 #185 #USD/kWh
# CAPEX_BESS_inverter = 0 #48 #USD/kWp
# CAPEX_PV_inverter = 48 #48 #USD/kWp
CAPEX_pv = 388 #USD/kWp
...
#m.addConstr(C == 80000, name='r10') 
#m.addConstr(PinverterBESS == 20000, name='r11')
m.addConstr(Ppvinst    == 100000, name='r12')
...
# for t in T:
#     m.addConstr(Pc[t] <= Ppv[t], name='r19')
#     m.addConstr(Pc[t] >= 0.1*Ppv[t], name='r19')
m.addConstr(AvG==quicksum(data[t]['lambda'] for t in T)/8760, name='r18') 
...
if m.Status == GRB.OPTIMAL:
    ...
    LCOEgross = 1000*(Io + OPEXgross/crfe)/(Ws.X/crf) if Ws.X > 0 else 0
    LCOEnet = 1000*(Io + (OPEX.X - OPEX0.X - Es.X)/crfe)/(Ws.X/crf) if Wl.X > 0 else 0
